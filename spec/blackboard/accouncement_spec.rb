  ...
  describe '#get announcements' do
    it 'return announcements list from the blackboard LMS' do
      ...
      mock_response = {
        "results": [
          {
            "id": "announcement_string",
            "title": "Alert Extended Due Date",
      ...
      # Verify
      expect(announcements.first['id']).to eq("announcement_string")
      expect(announcements.first['creatorUserId']).to eq("mrgutie8")
    end
  end
end